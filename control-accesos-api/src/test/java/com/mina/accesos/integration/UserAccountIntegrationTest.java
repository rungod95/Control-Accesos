...
public class UserAccountIntegrationTest extends IntegrationTestSupport {
    ...
    @Test
    void createUserFailsWhenRoleIsInvalid() throws Exception {
        String adminToken = obtainAdminToken();
        ...
        mockMvc.perform(post("/api/users")
                ...
                .andExpect(jsonPath("$.error").value("Rol no soportado: gerente"));
    }
    ...
    @Test
    void updateUserFailsWhenRoleIsInvalid() throws Exception {
        String adminToken = obtainAdminToken();
        String username = "inttest_invalidrole_" + UUID.randomUUID().toString().substring(0, 6);
        createdUsernames.add(username);

        UserAccount created = userAccountRepository.save(buildUser(username, "TRABAJADOR", "Cambio"));
        ...
        mockMvc.perform(put("/api/users/{id}", created.getId())
                        ...
                        .content(payload))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error").value("Rol no soportado: invalido"));
    }
    ...
    @Test
    void createUserFailsWhenRequiredFieldsMissing() throws Exception {
        String adminToken = obtainAdminToken();
        ...
        mockMvc.perform(post("/api/users")
                ...
                .andExpect(jsonPath("$.detalle", hasItem("username: username es obligatorio")))
                .andExpect(jsonPath("$.detalle", hasItem("password: password es obligatorio")))
                .andExpect(jsonPath("$.detalle", hasItem("role: role es obligatorio")));
    }

    @Test
    void updateUserFailsWhenRoleMissing() throws Exception {
        String adminToken = obtainAdminToken();
        String username = "inttest_missingrole_" + UUID.randomUUID().toString().substring(0, 6);
        createdUsernames.add(username);

        UserAccount created = userAccountRepository.save(buildUser(username, "TRABAJADOR", "Validación"));
